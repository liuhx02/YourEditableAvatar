...
from tetgs_inpainter.cameras import camera_utils
from tetgs_inpainter.cameras.rays import RayBundle
from utils.tensor_dataclass import TensorDataclass
from tetgs_inpainter.cameras.camera_utils import SO3toR3, R3toSO3
...
class Cameras(TensorDataclass):
    ...
    mvps: Optional[TensorType["num_cameras":..., 4, 4]]
    ...
    def __init__(
        ...
        mvps: Optional[TensorType["batch_mvps":..., 4, 4]] = None,
        ...
        # fx fy calculation
        self.fx = self._init_get_fc_xy(fx, "fx")  # type: ignore
        self.fy = self._init_get_fc_xy(fy, "fy")  # type: ignore

        # cx cy calculation
        self.cx = self._init_get_fc_xy(cx, "cx")  # type: ignore
        self.cy = self._init_get_fc_xy(cy, "cy")  # type: ignore
        
        # mvps calculation
        self.mvps = self._init_get_mvps(mvps)  # type: ignore
        ...
        self.camera_type = self._init_get_camera_type(camera_type)  # type: ignore
        self.times = self._init_get_times(times)  # type: ignore
    ...
    def _init_get_mvps(self, mvps):
        if mvps is None:
            mvps = None
        elif isinstance(mvps, torch.Tensor):
            mvps = mvps.to(self.device)
        else:
            raise ValueError(f"mvps must be a tensor, got {type(mvps)}")
        return mvps
    ...
    def _init_get_camera_type(
        ...
        if isinstance(camera_type, CameraType):
            camera_type = torch.tensor([camera_type.value], device=self.device)  # type: ignore
        elif isinstance(camera_type, List) and isinstance(camera_type[0], CameraType):
            camera_type = torch.tensor([[c.value] for c in camera_type], device=self.device)  # type: ignore
        elif isinstance(camera_type, int):
            camera_type = torch.tensor([camera_type], device=self.device)  # type: ignore
            ...
            camera_type = camera_type.to(self.device)  # type: ignore
        ...
        return camera_type  # type: ignore
    ...
    def _init_get_height_width(
        ...
        if isinstance(h_w, int):
            h_w = torch.Tensor([h_w]).to(torch.int64).to(self.device)  # type: ignore
            ...
            h_w = h_w.to(torch.int64).to(self.device)  # type: ignore
        ...
        # assert torch.all(h_w == h_w.view(-1)[0]), "Batched cameras of different h, w will be allowed in the future."
            ...
            h_w = torch.Tensor((c_x_y * 2).to(torch.int64).to(self.device))  # type: ignore
        else:
            raise ValueError("Height must be an int, tensor, or None, received: " + str(type(h_w)))
        return h_w  # type: ignore
    ...
    def get_image_coords(
        ...
        if index is None:
            ...
            image_coords = torch.meshgrid(torch.arange(image_height), torch.arange(image_width), indexing="ij")  # type: ignore
            ...
            image_coords = torch.stack(image_coords, dim=-1) + pixel_offset  # stored as (y, x) coordinates
        return image_coords  # type: ignore

    def generate_rays(  # pylint: disable=too-many-statements
        self,
        camera_indices: Union[TensorType["num_rays":..., "num_cameras_batch_dims"], int],
        coords: Optional[TensorType["num_rays":..., 2]] = None,  # type: ignore
        camera_opt_to_camera: Optional[TensorType["num_rays":..., 6]] = None,
        ...
        to generate the rays. Our goal is to parse the arguments and then get them into the right shape:
            ...
            - coords: (num_rays:..., 2)
            - camera_opt_to_camera: (num_rays:..., 6) or None
        ...
        if isinstance(camera_indices, torch.Tensor) and isinstance(coords, torch.Tensor):
            ...
            assert camera_opt_to_camera is None or camera_opt_to_camera.shape[:-1] == num_rays_shape, errormsg
        ...
        if isinstance(camera_indices, int):
            ...
            camera_indices = torch.tensor([camera_indices], device=cameras.device)  # type: ignore
        ...
        if cameras.is_jagged and coords is None and (keep_shape is None or keep_shape is False):
            ...
            camera_indices = torch.cat(  # type: ignore
        ...
        if coords is None:
            ...
            coords = coords.reshape(coords.shape[:2] + (1,) * len(camera_indices.shape[:-1]) + (2,))  # (h, w, 1..., 2)  # type: ignore
            coords = coords.expand(coords.shape[:2] + camera_indices.shape[:-1] + (2,))  # (h, w, num_rays, 2)  # type: ignore
            camera_opt_to_camera = (  # (h, w, num_rays, 6) or None
                camera_opt_to_camera.broadcast_to(coords.shape[:-1] + (6)) # type: ignore
                if camera_opt_to_camera is not None
                else None
            )
            distortion_params_delta = (  # (h, w, num_rays, 6) or None   # type: ignore
        ...
        assert camera_opt_to_camera is None or camera_opt_to_camera.shape[:-1] == coords.shape[:-1]
        ...
        raybundle = cameras._generate_rays_from_coords(
            camera_indices, coords, camera_opt_to_camera, distortion_params_delta, disable_distortion=disable_distortion  # type: ignore
        )
    ...
    def _generate_rays_from_coords(
        ...
        camera_indices = camera_indices.to(self.device)  # type: ignore
        coords = coords.to(self.device)  # type: ignore
        ...
        assert camera_opt_to_camera is None or camera_opt_to_camera.shape == num_rays_shape + (6,)
        ...
        if camera_opt_to_camera is not None:
            ...
            rvecs = SO3toR3(c2w_3)  # type: ignore
            rvecs = rvecs.reshape(-1, 3)
            rvec_opt = camera_opt_to_camera[..., 3:]
            trans_opt = camera_opt_to_camera[..., :3]
            
            # PORF: comparable results with alpha in range [1e-6, 1e-2]
            alpha = 1e-5
            rvecs_new = rvecs + alpha * rvec_opt
            trans_new = trans + alpha * trans_opt
            rot_new = R3toSO3(rvecs_new)  # type: ignore
            rot_new = rot_new.reshape(-1, 3, 3)
        ...
        return RayBundle(
            origins=origins,  # type: ignore
            directions=directions,  # type: ignore
            pixel_area=pixel_area,  # type: ignore
    ...
    def to_json(
        ...
        json_ = {
            ...
            "times": flattened[camera_idx].times.item() if self.times is not None else None,  # type: ignore
            "mvps": self.mvps[camera_idx].tolist() if self.mvps is not None else None,
        }
    ...
    def rescale_output_resolution(
        ...
        if isinstance(scaling_factor, (float, int)):
            scaling_factor = torch.tensor([scaling_factor]).to(self.device).broadcast_to((self.cx.shape))  # type: ignore
        elif isinstance(scaling_factor, torch.Tensor) and scaling_factor.shape == self.shape:
            scaling_factor = scaling_factor.unsqueeze(-1)  # type: ignore
        ...
        self.fx = self.fx * scaling_factor  # type: ignore
        self.fy = self.fy * scaling_factor  # type: ignore
        self.cx = self.cx * scaling_factor  # type: ignore
        self.cy = self.cy * scaling_factor  # type: ignore
        self.height = (self.height * scaling_factor).to(torch.int64)  # type: ignore
        self.width = (self.width * scaling_factor).to(torch.int64)  # type: ignore
        
        if self.mvps is not None:
            # TODO: add mvps translation
            self.near = 0.05
            self.far = 1000  # infinite
            aspect = self.width[0] / self.height[0]
            projections = []
            for i in range(self.fy.shape[0]):
                ...
                c_x = self.cx[i]
                c_y = self.cy[i]
                y = self.height[i] / (2.0 * fl_y.item()) # fy
                ...
                # ], dtype=torch.float32)
                projection = torch.tensor([
                    [2.0 * fl_x / self.width[i], 0, (self.width[i] - 2.0 * c_x) / self.width[i], 0],
                    [0, -2.0 * fl_y / self.height[i], (self.height[i] - 2.0 * c_y) / self.height[i], 0],
                    [0, 0, -(self.far + self.near) / (self.far - self.near), -2.0 * self.far * self.near / (self.far - self.near)],
                    [0, 0, -1, 0]
                ], dtype=torch.float32)
                projections.append(projection)
            projections = torch.stack(projections) # [N, 4, 4]
            
            extra_row = torch.tensor([[0, 0, 0, 1]], dtype=torch.float32).repeat(self.camera_to_worlds.shape[0], 1, 1)
            c2w_tmp = torch.cat((self.camera_to_worlds, extra_row), dim=1)
            self.mvps = projections @ torch.inverse(c2w_tmp)  # type: ignore
            print(self.mvps.shape)  # type: ignore
