...
from utils.spherical_harmonics import eval_sh
...
'''
pos: 2d --> 2d + normal
opacity: 1d unlearnable --> 1d learnable
sh: sh_level=1 learnable
scale: 2d unlearnable --> 3d learnable
rot: 2d unlearnable --> 3d learnable
'''
class Edit3DTetGS(nn.Module):
    def __init__(
        self, 
        nerfmodel: GaussianSplattingWrapper,
        points: torch.Tensor,
        colors: torch.Tensor,
        edited_all_gaussians, # edited 2dgs gaussians
        ...
        *args, **kwargs) -> None:
        ...
        super(Edit3DTetGS, self).__init__()
        assert surface_mesh_to_bind is not None
        ...
        if surface_mesh_to_bind is not None:
            ...
            self.bind_3dgs = True   # from False to True
            self.fixed_2dgs = False   # from True to False
            ...
            self.all_faces_num = self._surface_mesh_faces.shape[0]
            # all vertices num
            all_vertices = torch.tensor(np.array(surface_mesh_to_bind.vertices)).float().to(nerfmodel.device)
            self.all_vertices_num = self._verts_points.shape[0]
            # TODO: fix
            ...
            update_normal = True and self.bind_3dgs
            self.update_normal = update_normal
            if self.update_normal:
                edit_points = edited_all_gaussians["edit_xyz"].to(nerfmodel.device)
                edit_face_indices = edited_all_gaussians["edit_face_indices"].to(nerfmodel.device)
                self._edit_face_indices = edit_face_indices
                
                n_edit_points = edit_points.shape[0]
                self._n_edit_points = n_edit_points
                self.ori_edit_points = torch.nn.Parameter(
                    edit_points.to(nerfmodel.device),
                    requires_grad=False).to(nerfmodel.device)
                
                # normal
                edit_verts_normals = torch.nn.functional.normalize(self._edit_surface_mesh.verts_normals_list()[0], dim=-1).reshape(-1, 3)
                print('edit normal shape: ', edit_verts_normals.shape)
                faces_normals = edit_verts_normals[self._edit_mesh_faces]
                normals, _ = self.calculate_attr_by_bary_coords(faces_normals[:, None])
                self._edit_normals = torch.nn.Parameter(
                    normals.to(nerfmodel.device),
                    requires_grad=False).to(nerfmodel.device)
                
                # delta
                deltas = torch.zeros(self._n_edit_points, 1).to(nerfmodel.device)
                self._edit_points = nn.Parameter(
                    deltas, 
                    requires_grad=self.learn_positions
                ).to(nerfmodel.device)
                self._edit_face_indices = nn.Parameter(
                    edit_face_indices,
                    requires_grad=False
                ).to(nerfmodel.device)
                print("edit point shape: ", self._edit_points.shape)
            else:
        ...
        self.fx = self.nerf_cameras.fx[0].item()
        self.fy = self.nerf_cameras.fy[0].item()
        ...
        # density
        if not self.learn_opacities:
            all_edit_densities = inverse_sigmoid(0.9999 * torch.ones((n_edit_points, 1), dtype=torch.float, device=nerfmodel.device))
            self.learn_opacities = False
        else:
            all_edit_densities = edited_all_gaussians["edit_opacities"].to(nerfmodel.device)
        ...
        if self.bind_3dgs:
            ...
            edit_scales = edited_all_gaussians["edit_scales"].to(nerfmodel.device)
            edit_quaternions = edited_all_gaussians["edit_rots"].to(nerfmodel.device)
            self._edit_scales = nn.Parameter(
                edit_scales,
                requires_grad=self.learn_scales
            ).to(nerfmodel.device)
            print("self._edit_scales shape: ", self._edit_scales.shape)
            self._edit_quaternions = nn.Parameter(
                edit_quaternions,
                requires_grad=self.learn_quaternions
            ).to(nerfmodel.device)
            print("self._edit_quaternions shape: ", self._edit_quaternions.shape)
        else:
            raise NotImplementedError
        
        # Initialize color features
        self.ori_edit_sh_levels = edited_all_gaussians["edit_sh_level"]
        self.edit_sh_levels = sh_levels
        edit_sh_coordinates_dc = edited_all_gaussians["edit_sh_coordinates_dc"].to(nerfmodel.device)
        self._edit_sh_coordinates_dc = nn.Parameter(
            edit_sh_coordinates_dc,
            requires_grad=True and (not freeze_gaussians)
        ).to(self.nerfmodel.device)
        if self.edit_sh_levels > 1:
            if self.ori_edit_sh_levels == self.edit_sh_levels:
                edit_sh_coordinates_rest = edited_all_gaussians["edit_sh_coordinates_rest"].to(nerfmodel.device)
            else:
                edit_sh_coordinates_rest = torch.zeros(self._n_edit_points, self.edit_sh_levels**2 - 1, 3).to(nerfmodel.device)
            
            self._edit_sh_coordinates_rest = nn.Parameter(
                edit_sh_coordinates_rest,
                requires_grad=True and (not freeze_gaussians)
            ).to(self.nerfmodel.device)
            
        # =========== load reconstruction gaussian attrs ============
        keep_points = edited_all_gaussians["keep_xyz"].to(nerfmodel.device)
        keep_opacities = edited_all_gaussians["keep_opacities"].to(nerfmodel.device)
        keep_scales = edited_all_gaussians["keep_scales"].to(nerfmodel.device)
        keep_quaternions = edited_all_gaussians["keep_rots"].to(nerfmodel.device)
        keep_sh_coordinates_dc = edited_all_gaussians["keep_sh_coordinates_dc"].to(nerfmodel.device)
        keep_face_indices = edited_all_gaussians["keep_face_indices"].to(nerfmodel.device)
        keep_sh_levels = edited_all_gaussians["keep_sh_level"]
        if keep_sh_levels > 1:
            keep_sh_coordinates_rest = edited_all_gaussians["keep_sh_coordinates_rest"].to(nerfmodel.device)
    ...
    @property
    def points(self):
        # add normal offset for edit_points
        if self.update_normal:
            new_edit_pos = self.ori_edit_points + self._edit_normals * self._edit_points
            full_points = torch.cat([self._keep_points, new_edit_pos], dim=0)
            return full_points
        else:
            raise NotImplementedError
    ...
    @property
    def edit_scaling(self):
        if self.bind_3dgs:
            edit_scales = self.scale_activation(self._edit_scales)
            return edit_scales
        else:
            raise NotImplementedError
    ...
    def radii(self):
        ...
        gaussian_radii = radii[self._edit_face_indices.int()].reshape(-1)
    ...
    def render_image_gaussian_rasterizer(
        ...
        raster_settings = GaussianRasterizationSettings(
            ...
            sh_degree=sh_deg,   # not used if not compute color in rasterizer
            ...
            if not compute_color_in_rasterizer:
                if sh_rotations is None:
                    ...
                    edit_splat_colors = self.get_points_rgb(
                        ...
                        sh_levels=sh_deg + 1,   # current_sh_levels + 1
...
def load_refined_model(refined_tetgs_path, refined_tetgs_2dgs_path, nerfmodel:GaussianSplattingWrapper, edit_sh_levels=1):
    ...
    edited_2dgs_tetgs = load_inpainted_model(
        refined_tetgs_path=refined_tetgs_2dgs_path,
        nerfmodel=nerfmodel
    )
    editable_gaussians = convert_edited_tetgs_into_masked_gaussians(
        edited_tetgs=edited_2dgs_tetgs
    )
    
    # 初始化
    refined_tetgs = Edit3DTetGS(
        nerfmodel=nerfmodel,
        points=_points,
        colors=torch.ones((_points.shape[0], 3), dtype=torch.float).to(nerfmodel.device),
        edited_all_gaussians=editable_gaussians,
...
def convert_refined_tetgs_into_masked_gaussians_continuous(refined_tetgs: Edit3DTetGS, recon_mesh_path, edit_mesh_path, keep_faces_num):
    '''
    这回要一个一个面片的找对应关系了
    '''
    
    keep_gaussians = {}
    sh_level = refined_tetgs.keep_sh_levels
    
    with torch.no_grad():
        xyz = refined_tetgs.points.cpu().numpy()
        opacities = torch.cat([refined_tetgs.all_keep_densities, refined_tetgs.all_edit_densities], dim=0).cpu().numpy()
        scales = scale_inverse_activation(refined_tetgs.scaling).cpu().numpy()
        rots = refined_tetgs.quaternions.cpu().numpy()
        sh_coordinates_dc = torch.cat([refined_tetgs._keep_sh_coordinates_dc, refined_tetgs._edit_sh_coordinates_dc], dim=0).cpu().numpy()
        if sh_level > 1:
            sh_coordinates_rest = torch.cat([refined_tetgs._keep_sh_coordinates_rest, refined_tetgs._edit_sh_coordinates_rest], dim=0).cpu().numpy()
        # face_indices
        face_indices = torch.cat([refined_tetgs._keep_face_indices, refined_tetgs._edit_face_indices + keep_faces_num], dim=0).cpu().numpy()
        print("face_indices max: ", np.max(face_indices))
        
    # load recon_mesh and edit_mesh
    import trimesh
    trimesh_mesh = trimesh.load(recon_mesh_path, process=False)
    recon_mesh = o3d.geometry.TriangleMesh()
    recon_mesh.vertices = o3d.utility.Vector3dVector(trimesh_mesh.vertices)
    recon_mesh.triangles = o3d.utility.Vector3iVector(trimesh_mesh.faces)
    trimesh_edit_mesh = trimesh.load(edit_mesh_path, process=False)
    edit_mesh = o3d.geometry.TriangleMesh()
    edit_mesh.vertices = o3d.utility.Vector3dVector(trimesh_edit_mesh.vertices)
    edit_mesh.triangles = o3d.utility.Vector3iVector(trimesh_edit_mesh.faces)
    # prepare verts and faces
    recon_vertices = np.asarray(recon_mesh.vertices)
    recon_triangles = np.asarray(recon_mesh.triangles)
    edit_vertices = np.asarray(edit_mesh.vertices)
    edit_triangles = np.asarray(edit_mesh.triangles)
    # to tensor
    edit_vertices_tensor = torch.from_numpy(edit_vertices)
    recon_vertices_tensor = torch.from_numpy(recon_vertices)
    
    # 在recon_verts中找出现在edit_verts中的顶点
    block_size = 1000  # 定义块大小
    num_recon_vertices = recon_vertices_tensor.shape[0]

    # 初始化一个空的 mask
    mask = torch.zeros(num_recon_vertices, dtype=torch.int)
    
    masked_vertices_set = set(map(tuple, np.round(edit_vertices, decimals=2)))
    edit_mask = np.array([tuple(np.round(vertex, decimals=2)) in masked_vertices_set for vertex in recon_vertices])
    edit_mask = torch.from_numpy(edit_mask).int()
    print("edit_mask shape: ", edit_mask.shape)
    print("edit_mask sum: ", torch.sum(edit_mask))
    
    mask_np = edit_mask.numpy()
    hit_tri_id_keep_set = np.where((mask_np[recon_triangles[:, 0]] == 1) &
                                    (mask_np[recon_triangles[:, 1]] == 1) &
                                    (mask_np[recon_triangles[:, 2]] == 1))[0]
    # find keep gaussians
    mask_keep_faces = np.isin(face_indices, hit_tri_id_keep_set)
    keep_gaussian_indices = np.where(mask_keep_faces)[0] 
    ...
    print("keep_xyz shape: ", keep_xyz.shape)
    print("keep_opacities shape: ", keep_opacities.shape)
    print("keep_scales shape: ", keep_scales.shape)
    print("keep_rots shape: ", keep_rots.shape)
    print("keep_sh_coordinates_dc shape: ", keep_sh_coordinates_dc.shape)
    print("keep_face_indices shape: ", keep_face_indices.shape)
