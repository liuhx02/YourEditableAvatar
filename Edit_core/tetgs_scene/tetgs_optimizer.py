...
from .tetgs_model import TetGS
from .tetgs_edit_2d import EditTetGS
from .tetgs_edit_3d import Edit3DTetGS
...
class TetGSOptimizer():
    """Wrapper of the Adam optimizer used for TetGS optimization.
...
class EditTetGSOptimizer():
    """Wrapper of the Adam optimizer used for EditTetGS optimization.
    Largely inspired by the original implementation of the 3D Gaussian Splatting paper:
    https://github.com/graphdeco-inria/gaussian-splatting
    """
    def __init__(
        ...
        if (model.binded_to_surface_mesh and model.learn_positions):
            l = [{'params': [model._edit_points], 'lr': opt.position_lr_init * spatial_lr_scale, "name": "points"}]
        else:
            l = []

        if not model.freeze_gaussians:
            l = l + [{'params': [model._edit_sh_coordinates_dc], 'lr': opt.feature_lr, "name": "sh_coordinates_dc"},]
            if model.edit_sh_levels > 1:
                l = l + [{'params': [model._edit_sh_coordinates_rest], 'lr': opt.feature_lr / 20.0, "name": "sh_coordinates_rest"}]
            
        if model.learn_opacities:
            l = l + [{'params': [model.all_edit_densities], 'lr': opt.opacity_lr, "name": "all_densities"}]
        if (model.binded_to_surface_mesh and model.learn_scales):
            l = l + [{'params': [model._edit_scales], 'lr': opt.scaling_lr, "name": "scales"}]
        if (model.binded_to_surface_mesh and model.learn_scales):
            l = l + [{'params': [model._edit_quaternions], 'lr': opt.rotation_lr, "name": "quaternions"}]
