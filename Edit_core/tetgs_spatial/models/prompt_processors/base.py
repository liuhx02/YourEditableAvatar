...
from tetgs_spatial.utils.misc import barrier, cleanup, get_rank
from tetgs_spatial.utils.ops import shifted_cosine_decay, shifted_expotional_decay
...
class PromptProcessor(BaseObject):
    ...
    def configure(self) -> None:
        self._cache_dir = ".tetgs_spatial_cache/text_embeddings"  # FIXME: hard-coded path
        ...
        tetgs_spatial.info(f"Using view-dependent prompts {prompts_vd_display}")
    ...
    def load_text_embeddings(self):
        ...
        tetgs_spatial.debug(f"Loaded text embeddings.")
    ...
    def preprocess_prompt(self, prompt: str) -> str:
        if prompt.startswith("lib:"):
            ...
            tetgs_spatial.info("Find matched prompt in library: " + candidate)
            return candidate
        else:
            return prompt
    ...
    def get_debiased_prompt(self, prompt: str) -> List[str]:
        ...
        tetgs_spatial.info(f"Words that can potentially be removed: {words_to_debias}")
        ...
        for d, debiased_prompt in zip(views, debiased_prompts):
            tetgs_spatial.info(f"Debiased prompt of the {d} view is [{debiased_prompt}]")
